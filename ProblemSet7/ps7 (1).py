...
def filterStories(stories, triggerlist):
    ...
    result = []
    for item in stories:
        for test in triggerlist:
            if test.evaluate(item):
                result.append(item)
                break
...
def makeTrigger(triggerMap, triggerType, params, name):
    ...
    """
    #TITLE,SUBJECT,SUMMARY,NOT,AND,OR,PHRASE
    if triggerType.upper() == "TITLE":
        temp = TitleTrigger(params[0])
    elif triggerType.upper() == "SUBJECT":
        temp = SubjectTrigger(params[0])
    elif triggerType.upper() == "SUMMARY":
        temp = SummaryTrigger(params[0])
    elif triggerType.upper() == "NOT":
        temp = NotTrigger(triggerMap[params[0]])
    elif triggerType.upper() == "AND":
        temp = AndTrigger(triggerMap[params[0]],triggerMap[params[1]])
    elif triggerType.upper() == "OR":
        temp = OrTrigger(triggerMap[params[0]],triggerMap[params[1]])
    elif triggerType.upper() == "PHRASE":
        temp = PhraseTrigger(' '.join(params))
    else:
        return None
    
    triggerMap[name] = temp
    return temp
